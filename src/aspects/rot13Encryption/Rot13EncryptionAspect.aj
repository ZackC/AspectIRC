...
public aspect Rot13EncryptionAspect {
	...
	void around(Client c, Message msg): this(c) && execution(public void send(Message)) && args(msg){
		if (rot13EncyrptionCheckBox != null
				&& rot13EncyrptionCheckBox.isSelected()) {
			...
			msg = new Message(newHeader, alterString(msg.getContent()
					.toLowerCase()));
			System.out.println("rot13 encrypted message: " + msg.getContent());
			System.out.println("rot13 encrypted header: " + msg.getHeader());
	...
	void around(Client c, Object msg): this(c) && execution(protected void handleIncomingMessage(Object)) && args(msg){
		if (msg instanceof Message) {
			...
			if (headerPos != -1) {
				System.out
						.println("rot13 header recieved: " + msg1.getHeader());
				...
				if (msg1.getHeader().length() > headerPos + 7) {
					...
					header = header.substring(0, headerPos)
							+ header.substring(headerPos + 7);
				}
